...
export const ModelUI = ({ hidden, ...props }) => {
  const input = useRef();
    ...
    <>
      <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
        ...
        <div className="flex items-center gap-2 pointer-events-auto max-w-screen-sm w-full mx-auto">
          <input
            className="w-full p-4 rounded-md bg-white text-[#c3582a] placeholder-[#f77035] border border-[#c3582a] focus:border-[#f77035] focus:outline-none"
          ...
          <button
            onClick={() => sendMessage()}
            className="bg-[#f77035] hover:bg-[#c3582a] text-white p-4 px-5 font-semibold uppercase rounded-md"
          >
            Send
          </button>
          <button
            onClick={startListening}
            className={`pointer-events-auto bg-[#f77035] hover:bg-[#c3582a] text-white p-4 rounded-md ${
              isListening ? "hover:bg-[#aa4f28]" : ""
            }`}
          >
            {isListening ? (
              <svg
